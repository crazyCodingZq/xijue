...
import com.zq.xijue.util.AliyunSmsUtils;
...
public class ApiController {
    ...
    @PostMapping(SecurityConstants.VALIDATE_CODE_URL_PREFIX + "/sms")
    public String sms(String mobile, HttpSession session) {
        try {
            int loginTime = SysCache.get(mobile);
            if (loginTime > 5) {
                return "请稍后再试";
            } else {
                SysCache.put(mobile);
            ...
            SysCache.mobileCodeCache.put(mobile, code + "");
